function [predictionsTr, predictionsTe, rmseStart, rmseEnd, rmseBest, U, M, rmseTrVector, rmseTeVector, rmseTrVector2, rmseTeVector2] = exp01modelLambdaLS(Ytr, Yte, Itr, Ite, G, seed, numOfLatentFactors, lambdaU, lambdaM, lambdaT, friendshipIncluded, numOfIterations)

    expTr = expSparseMatrix(Ytr, Itr);
    expTe = expSparseMatrix(Yte, Ite);
    ...
    rmseTrVector2 = zeros(numOfIterations, 1);
    rmseTeVector2 = zeros(numOfIterations, 1);
    ...
    for iteration=1:numOfIterations
        ...
        Ptr = expSparseMatrix(predictionsTr, Itr);
        Pte = expSparseMatrix(predictionsTe, Ite);
        
        rmseTrVector2(iteration) = calculateRMSE(Ptr, expTr, Itr);
        rmseTeVector2(iteration) = calculateRMSE(Pte, expTe, Ite);
