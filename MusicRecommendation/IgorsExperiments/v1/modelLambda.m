function [rmseStart, rmseEnd, rmseBest] = modelLambda(Ytr, Yte, G, seed, numOfLatentFactors, lambda, lambdaT, friendshipIncludedInTheModel, alpha, numOfIterations)
    ...
    if ((abs(rmseTr1 - rmseTr2) < 0.001)&&(abs(rmseTr2 - rmseTr3) < 0.001))
    ...
    for iteration=1:numOfIterations
        ...
        for i=1:numOfUsers
            ...
            if (friendshipIncludedInTheModel == 1)
                g = -M * dg * Ri' + (MMt + ni * lfeye) * Ui + lambdaT * (Ui - U * G(:, i));     % added for friendship graph
                H = MMt + ni * lfeye + lfTeye;
            else
                g = -M * dg * Ri' + (MMt + ni * lfeye) * Ui;
                H = MMt + ni * lfeye;
            end
