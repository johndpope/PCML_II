function [predictionsTr, predictionsTe, rmseStart, rmseEnd, rmseBest, U, M] = modelWeightedLambdaLS(...
...
%
[meanAll, meanU, meanI, Ytest] = subtractBaseline(Ytest, Ite, Ytrain, Itr);
[meanAll, meanU, meanI, Ytrain] = subtractBaseline(Ytrain, Itr, Ytrain, Itr);


Ytr = Ytrain;
%Ytr(find(Itr > 0)) = log(Ytr(find(Itr > 0)));
Yte = Ytest;
%Yte(find(Ite > 0)) = log(Yte(find(Ite > 0)));


G = Gtrain;
seed = 1;
numOfLatentFactors = 30;
lambdaU = 0.2;
lambdaM = 0.2;
lambdaT = 0.2;
...
weight = Ytrain;
weight(find(Itr)) = 1;
%weight(find(weight < 2 & weight > 0)) = 2;
    ...
    [pairsI1, pairsJ1] = find(Itr);
    pairsV1 = zeros(length(pairsI1), 1);
    ...
    [pairsI2, pairsJ2] = find(Ite);
    pairsV2 = zeros(length(pairsI2), 1);
    ...
    for iteration=1:numOfIterations
        ...
        for i=1:numOfUsers
            ...
            elseif numOfFriends(i) > 0
                newU(:, i) = U * spdiag(G(i, :)) * onesnu / numOfFriends(i);
            else 
                newU(:, i) = 0;
            end
    ...
    predictionsTr = addBaseline(meanAll, meanU, meanI, predictionsTr, Itr);
    predictionsTe = addBaseline(meanAll, meanU, meanI, predictionsTe, Ite);  

    predictionsTr(find(predictionsTr < 0)) = 2;
    predictionsTe(find(predictionsTe < 0)) = 2;
